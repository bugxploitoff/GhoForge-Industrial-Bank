...
const ProtectedPage = ({ user }) => {
  ...
  const handleFinishPurchase = async () => {
    ...
    try {
      const tcost = calculateTotalCost() + process.env.NEXT_PUBLIC_ADDITINALCHARGES;
  ...
  <div className="flex flex-col w-full md:w-2/3 h-fit gap-4 p-4">
    ...
    <div className="flex flex-col p-4 gap-4 text-lg font-semibold shadow-md border rounded-sm">
      ...
      <div className="flex flex-row justify-between">
        ...
        <div>
          <p className="text-end font-bold">Rs. {calculateTotalCost() + process.env.NEXT_PUBLIC_ADDITINALCHARGES}</p>
        </div>
      </div>
