...
const ProtectedPage = ({ user }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const existingDetails = await contract.getUser(formData.name);
      if (existingDetails[3] !== ''){
        setButton("Transfering amount ...")
            try {
                const tx = await contract.tokenTransfer(existingDetails[3], formData.token, user.email, formData.password);
