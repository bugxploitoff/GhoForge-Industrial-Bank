...
const register = () => {
    ...
    const [isWalletGet, setIsWalletGet] = useState("Connect your wallet")
  ...
  useEffect(() => {
    setEmailData((prevEmailData) => ({
      ...prevEmailData,
      to: formData.email,
      text: `Your account has been created successfully`, // Added a colon after "password"
    }));
      ...
      const handleFormSubmit = async (e: React.FormEvent) => {
        ...
        try {
        const existingDetails = await contract.getUser(formData.email);
        if (existingDetails[2] === '') {
            try {
              // Deploy the contract and wait for the transaction to be mined
              const deploymentTx = await contract.userRegister(
                formData.name,
                formData.email,
                formData.email,
                address,
